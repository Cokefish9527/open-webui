...
async def lifespan(app: FastAPI):
    ...
    # 注册对话消息队列处理器
    register_conversation_queue_handler(redis_queue_listener)
    # 初始化并启动队列监听器
    ...
    # 停止Redis队列监听器
    if hasattr(app.state, "redis_queue_monitoring_task"):
        app.state.redis_queue_monitoring_task.cancel()
        await redis_queue_listener.stop_monitoring()
