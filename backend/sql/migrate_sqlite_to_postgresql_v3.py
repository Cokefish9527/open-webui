...
def convert_value_for_postgresql(value, column_name, table_name):
    ...
    timestamp_columns = [
        ...
        'processed_at', 'learned_at', 'last_stats_update_at'
    ]
    ...
    boolean_columns = [
        ...
        'is_org_admin', 'active', 'archived', 'is_expanded'
    ]
...
def import_table_data_postgresql(conn, table_name, columns, rows):
    ...
    try:
        # 清空目标表数据（避免重复键冲突）
        cursor.execute(f'DELETE FROM {pg_table_name}')
        print(f"  已清空表 {table_name} 的现有数据")
    except Exception as e:
        print(f"  清空表 {table_name} 数据时出错: {e}")
