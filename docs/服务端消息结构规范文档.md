...
字段说明：
- `type`: 消息类型，可选值："chat"(聊天消息) | "workflow_trigger"(工作流触发)
- `content`: 用户输入的对话文字内容
- `user_id`: 用户唯一标识符
- `session_id`: 会话ID，用于维持对话上下文（可选）
- `entry_type`: 对话入口类型，可选值："chat"(普通聊天) | "company"(公司信息) | "business"(商业分析) | "video_publish"(视频发布)（可选）
- `task_id`: 关联的任务ID（可选）
- `metadata`: 元数据信息（可选）
  - `timestamp`: 时间戳
...
字段说明：
- `type`: 消息类型，固定为"hsai_response"
- `success`: 是否成功，布尔值
- `execution_id`: 执行ID，用于唯一标识一次工作流调用
- `session_id`: 会话ID
- `user_id`: 用户ID
- `execution_time`: 执行时间，使用时间字符串格式，如"0.00s"
- `timestamp`: 时间戳，使用标准的unixtime格式
- `messageType`: 消息类型，参考Agent2Redis消息体的content_type字段，1-processing, 2-pre_text, 3-text, 4-thinking, 5-result, 6-selection
- `displayText`: 显示文本
- `data`: 具体数据内容，包含以下字段：
...
- `status`: 状态，使用Agent2Redis的status字段，值为RUNNING或FINISHED
