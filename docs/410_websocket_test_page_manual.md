...
### 3.7 任务调试面板
1. 打开“任务调试”标签后，左侧卡片展示企业名称、项目名称以及主任务列表，首次进入请点击“刷新快照”以获取 `/api/v1/hsai/tasks`、`/api/v1/hsai/companies`、`/api/v1/hsai/projects` 的最新数据。
2. 事件流默认订阅 `task_status_updated`、`task_progress`、`task_replay` 三类 Socket 事件，可通过筛选下拉与“仅看当前会话”开关快速定位关键信息。
3. “填充默认主任务” 会针对当前项目调用 `POST /api/v1/hsai/tasks` 写入 `MAIN_TASK_TEMPLATES` 中定义的模板，适合初始化新项目。
4. “标记完成 / 恢复执行中” 使用 `PUT /api/v1/hsai/tasks/{task_id}` 更新主任务状态；操作后自动刷新快照以校准界面。
5. 周期任务支持 `POST /api/v1/hsai/tasks/{task_id}/start` 立即触发，也可通过日期选择框模拟调度，会在事件流中追加本地回放记录。
6. 右侧子任务列表依赖主任务选择，支持勾选后“回放选中子任务”，用于记录现场回放或向后端复现问题。
