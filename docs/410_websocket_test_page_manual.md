...
### 3.7 任务系统
1. 切换到“任务系统”标签后，概览卡片会展示企业名称、项目名称以及主线完成度与循环任务状态。首次进入请点击“刷新概览”，依次请求 `/api/v1/hsai/tasks`、`/api/v1/hsai/companies`、`/api/v1/hsai/projects` 获取最新数据。
2. 事件面板默认订阅 `task_status_updated`、`task_progress`、`task_replay` 三类 Socket 事件，可使用类型下拉框与“仅当前会话”开关快速筛查调试过程中的关键记录。
3. “创建默认主线任务”按钮会针对当前项目调用 `POST /api/v1/hsai/tasks` 写入 `MAIN_TASK_TEMPLATES` 中定义的模板，适合初始化项目或回归测试。
4. 主线任务操作区提供“标记主线完成”“重置为未开始”两个入口，对应 `PUT /api/v1/hsai/tasks/{task_id}` 接口；执行后页面会自动刷新概览并在事件面板留存结果。
5. 循环任务支持 `POST /api/v1/hsai/tasks/{task_id}/start` 激活计划，同时可通过“模拟调度日期”输入框+按钮生成当日子任务，验证调度逻辑及任务依赖。
6. 子任务列表会收集最近生成的执行项，选中后点击“回放选中子任务”即可将 payload 写回会话事件，便于复盘或再次触发。
...
## 7. 变更对齐
- 代码：详见 `websocket-test.html`（消息卡片布局、任务调试标签结构）与 `static/ws-tester.js`（任务快照刷新、事件分发、操作按钮状态机）。
