...
### 预期协同模式
```
客户端 ←→ Socket.IO ←→ OpenWebUI服务端 ←→ n8n Webhook ←→ n8n工作流
                                            ↕
                                       Redis信号机制
```

**核心流程**:
1. 客户端通过OpenWebUI原生Socket.IO与服务端直连
2. 服务端接收客户端对话消息
3. 服务端转发消息到对应的n8n webhook
4. n8n处理后返回结果（字符串格式）
5. 服务端进行结构化处理
6. 通过Socket.IO返回给客户端
7. 对于长任务，通过Redis信号进行实时状态同步
...
- **触发方式**: 主动调用/定时调用
- **主要任务**:
  - 抓取热门视频中头部几条并将链接写入数据库
  - 未确认的爆款链接以任务形式表现在工作台任务板块
  - 用户确认后，通知n8n进行爆款视频下载、脚本拆解、写入爆款库
