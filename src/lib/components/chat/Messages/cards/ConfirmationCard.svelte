...
>
	<div class="flex items-center justify-between mb-3">
		...
		{#if cardData?.taskId}
			<span
				class="text-xs px-2 py-1 bg-teal-100 dark:bg-teal-900 text-teal-800 dark:text-teal-200 rounded-full"
			>
	...
	{#if cardData}
		<div class="mb-3">
			<div class="font-medium text-gray-900 dark:text-white mb-2">确认发布以下内容</div>

			<div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3 mb-3">
				{#if cardData.platforms && cardData.platforms.length > 0}
					<div class="mb-2">
						<div class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">发布平台:</div>
				...
				{#if cardData.scheduleTime}
					<div class="mb-2">
						<div class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">发布时间:</div>
				...
				{#if cardData.contentPreview}
					<div>
						<div class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">内容预览:</div>
						<div
							class="text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-600 p-2 rounded"
						>
		...
		<div class="flex space-x-2">
			<Button variant="primary" size="sm" on:click={handleConfirm}>确认发布</Button>
			<Button variant="secondary" size="sm" on:click={handleCancel}>取消发布</Button>
		</div>
	{/if}
</div>
