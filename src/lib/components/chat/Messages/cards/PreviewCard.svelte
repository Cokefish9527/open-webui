...
>
	<div class="flex items-center justify-between mb-3">
		...
		{#if cardData?.taskId}
			<span
				class="text-xs px-2 py-1 bg-indigo-100 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200 rounded-full"
			>
	...
	{#if cardData}
		<div class="mb-3">
			<div class="font-medium text-gray-900 dark:text-white mb-2">预览内容</div>
		...
		<div class="flex space-x-2">
			<Button variant="primary" size="sm" on:click={handleApprove}>批准发布</Button>
			<Button variant="secondary" size="sm" on:click={handleReject}>需要修改</Button>
		</div>
	{/if}
</div>
