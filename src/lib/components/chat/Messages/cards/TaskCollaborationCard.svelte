<script lang="ts">
	...
	async function joinCollaboration() {
		...
		try {
			const success = await hsaiTaskService.addTaskCollaborator(
				task.id,
				currentUserId,
				'collaborator'
			);
...
>
	...
	{#if task}
		...
		<div class="mb-4">
					...
					{#each collaborators as collaborator}
						<div class="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-700 rounded">
							<div class="flex items-center">
								<div
									class="w-8 h-8 rounded-full bg-purple-500 flex items-center justify-center text-white text-xs"
								>
			...
			{:else}
				<div
					class="text-sm text-gray-500 dark:text-gray-400 p-4 text-center bg-gray-50 dark:bg-gray-700 rounded"
				>
					暂无协作者
				</div>
			{/if}
		...
		<div class="flex flex-wrap gap-2">
			{#if !isCollaborator && !isOwner}
				<Button on:click={joinCollaboration} class="flex-1" variant="primary" size="sm">
					加入协作
				</Button>
			{/if}

			{#if !isSharedToCurrentSession}
				<Button on:click={shareToSession} class="flex-1" variant="secondary" size="sm">
					共享到当前会话
				</Button>
			{/if}

			{#if isOwner}
				<Button class="flex-1" variant="outline" size="sm">管理协作者</Button>
			{/if}
		</div>
	{/if}
</div>
