<script lang="ts">
  ...
  function getPlatformInfo(platform: string) {
    return platformMap[platform as keyof typeof platformMap] || { label: platform, icon: '🔗', color: 'bg-gray-500 text-white' };
  }
  ...
  <div class="flex items-center justify-between mb-6 p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
    ...
    <div class="flex space-x-2">
      ...
      {:else if currentView === 'publish'}
        <button class="btn btn-primary" on:click={openPublishDialog}>
          创建发布任务
        </button>
      {/if}
            ...
            <div class="flex items-start justify-between mb-3">
              <div class="flex items-center space-x-3">
                {#if account.avatar_url}
                  <img src={account.avatar_url} alt={account.display_name} class="w-12 h-12 rounded-full" />
                {:else}
              ...
              <button
                ...
                on:click={() => syncAccount(account.id)}
              >
                同步
              </button>
            </div>
