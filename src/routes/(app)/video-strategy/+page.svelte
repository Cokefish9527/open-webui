	...
	let messages: Message[] = [
		{
			...
			sender: 'ai',
			content:
				'您好！我是您的外贸短视频策略助手，根据您设定的"北美市场拓展计划"，我为本周生成了3个短视频方案\n\n1. 产品拆解展示+使用场景演示\n2. 客户证言+生产过程揭秘\n3. 新品预告+限时促销活动\n\n您需要查看具体方案细节还是调整策略方向？',
			time: '10:30'
		},
		{
			id: 2,
			sender: 'user',
			content:
				'很好，请展示第一个方案的详细内容。另外，能分析下上周视频的数据表现吗？特别是北美地区的转化率情况。',
			time: '10:32',
			read: true
		},
		{
...
<div class="flex h-screen bg-gray-50 overflow-hidden">
	...
	<!-- Sidebar -->
	<aside
		class={`fixed inset-y-0 left-0 w-64 bg-gray-900 text-white transform transition-transform duration-300 ease-in-out z-40 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}
	>
				...
				<div class="flex-1 overflow-y-auto p-4 space-y-6 message-container">
					{#each messages as message}
						<div class="flex {message.sender === 'ai' ? 'justify-start' : 'justify-end'} group">
							<div
								class="max-w-3/4 rounded-2xl p-4 relative {message.sender === 'ai'
									? 'bg-white border border-gray-200'
									: 'bg-blue-500 text-white'}"
							>
								{#if message.sender === 'ai'}
									<div
										class="absolute -left-2 top-4 w-4 h-4 transform rotate-45 bg-white border-l border-t border-gray-200"
									></div>
								{:else}
									<div
										class="absolute -right-2 top-4 w-4 h-4 transform rotate-45 bg-blue-500"
									></div>
								{/if}
								...
								<div
									class="mt-1 text-xs {message.sender === 'ai'
										? 'text-gray-400'
										: 'text-blue-100'} flex items-center justify-end"
								>
											...
											{#each result.stats as stat}
												<div class="bg-gray-50 p-3 rounded-lg">
													...
													<div class="flex items-baseline mt-1">
														...
														{#if stat.change}
															<span
																class={`ml-2 text-xs font-medium ${stat.positive ? 'text-green-600' : 'text-red-600'}`}
															>
																{stat.change}
															</span>
														{/if}
								...
								>
									<div class="flex justify-between items-start">
										<div>
											...
											<div class="flex items-center mt-1">
												...
												<span
													class="ml-3 px-2 py-0.5 text-xs rounded-full {task.priority === '高'
									...
									<div class="mt-3">
										<div class="w-full bg-gray-200 rounded-full h-2">
											<div
												class="h-2 rounded-full {task.status === 'completed'
													? 'bg-green-500'
